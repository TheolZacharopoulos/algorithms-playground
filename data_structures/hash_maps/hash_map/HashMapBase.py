from abc import abstractmethod
from random import randrange

from data_structures.hash_maps import MapBase


class HashMapBase(MapBase):
    """Abstract base class for map using hash-table with MAD compression."""

    def __init__(self, capacity=11, p=109345121):
        """Create an empty hash-table map."""
        self._table = [None] * capacity

        # number of entries in the map
        self._n = 0

        # prime for MAD compression
        self._prime = p

        # scale from 1 to p-1 for MAD
        self._scale = 1 + randrange((p-1))

        # shift from 0 to p-1 for MAD
        self._shift = randrange(p)

    @abstractmethod
    def _bucket_getitem(self, j, k):
        """This method should search bucket j for an item having key k,
        returning the associated value, if found, or else raising a KeyError."""
        raise KeyError

    @abstractmethod
    def _bucket_setitem(self, j, k, v):
        """This method should modify bucket j so that key k becomes associated with value v.
        If the key already exists, the new value overwrites the existing value."""
        raise KeyError

    @abstractmethod
    def _bucket_delitem(self, j, k):
        """This method should remove the item from bucket j having key k,
        or raise a KeyError if no such item exists."""
        raise KeyError

    def _hash_function(self, k):
        """The MAD compression algorithm"""
        return (hash(k) * self._scale + self._shift) % self._prime % len(self._table)

    def __len__(self):
        return self._n

    def __getitem__(self, k):
        j = self._hash_function(k)

        # may raise KeyError
        return self._bucket_getitem(j, k)

    def __setitem__(self, key, value):
        j = self._hash_function(key)
        self._bucket_setitem(j, key, value)

        # subroutine maintains self._n
        # keep load factor <= 0.5
        if self._n > len(self._table) // 2:
            # number 2ˆx - 1 is often prime
            self._resize(2 * (len(self._table) - 1))

    def __delitem__(self, key):
        j = self._hash_function(key)
        # may raise KeyError
        self._bucket_delitem(j, key)
        self._n -= 1

    def _resize(self, c):
        """Resize bucket array to capacity c"""

        # use iteration to record existing items
        old = list(self.items())

        # then reset table to desired capacity
        self._table = c * [None]

        # n recomputed during subsequent adds
        self._n = 0

        for (k, v) in old:
            self[k] = v